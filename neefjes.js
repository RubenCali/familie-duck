...
const plaatjes  = document.querySelectorAll('.famDuck img');
const bolletjes = document.getElementById('bolletjes');

// hoogte gelijkmaken aan de eerste sectie
document.getElementsByClassName('info')[0].style.height = document.getElementsByClassName('famDuck')[0].clientHeight +'px';
...
for(let i=0; i<plaatjes.length; i++) {
    ...
    // plaatjes[i].aadEventListener('dblclick', bolltjes);
    // hier kun je nog 2 events aan toevoegen: click en dblclick
}

function vulKopAan() {
    document.getElementById('famDuck').innerText = ": "+this.title;
    document.getElementById('kop').style.color  = this.dataset.kleur;
}

function resetKop() {
    document.getElementById('famDuck').innerText = "";
